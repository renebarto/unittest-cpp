...
public:
    ...
    // delete[].  Returns the cloned std::string, or nullptr if the input is
    // nullptr.
    //
    // This is different from strdup() in std::string.h, which allocates
    // memory using malloc().
    ...
    // nullptr C std::string is considered different to any non-nullptr C std::string,
    // including the empty std::string.
    static bool CStringEquals(const char* lhs, const char* rhs);

    // Converts a wide C std::string to a String using the UTF-8 encoding.
    // nullptr will be converted to "(null)".  If an error occurred during
    // the conversion, "(failed to convert from wide std::string)" is
    // returned.
    ...
    // nullptr C std::string is considered different to any non-nullptr C std::string,
    // including the empty std::string.
    static bool CStringEquals(const wchar_t* lhs, const wchar_t* rhs);

    // Compares two C strings, ignoring case.  Returns true iff they
    // have the same content.
    //
    // Unlike strcasecmp(), this function can handle nullptr argument(s).
    // A nullptr C std::string is considered different to any non-nullptr C std::string,
    // including the empty std::string.
    ...
    // A nullptr C std::string is considered different to any non-nullptr wide C std::string,
    // including the empty std::string.
    ...
    // current locale.
    static bool CaseInsensitiveCStringEquals(const wchar_t* lhs, const wchar_t* rhs);

    // Returns true iff the given std::string ends with the given suffix, ignoring
    // case. Any std::string is considered to end with an empty suffix.
...
// uses UTF-16 for wide std::string encoding. All supported systems
...
std::string StringStreamToString(stringstream * ss)
{
    ...
    std::string result;
    result.reserve(2 * static_cast<uint64_t>(end - start));
...
// Converts a Unicode code point to a narrow std::string in UTF-8 encoding.
...
std::string CodePointToUtf8(uint32_t code_point)
{
    ...
    {
        return "(Invalid Unicode 0x" + ToNarrowString(String::FormatHexInt(static_cast<int>(code_point))) + ")";
    }
...
// Converts a wide std::string to a narrow std::string in UTF-8 encoding.
// The wide std::string is assumed to have the following encoding:
//   UTF-16 if sizeof(wchar_t) == 2 (on Windows, Cygwin, Symbian OS)
//   UTF-32 if sizeof(wchar_t) == 4 (on Linux)
// Parameter str points to a null-terminated wide std::string.
// Parameter num_chars may additionally limit the number
// of wchar_t characters processed. -1 is used when the entire std::string
// should be processed.
// If the std::string contains code points that are not valid Unicode code points
// (i.e. outside of Unicode range U+0 to U+10FFFF) they will be output
// as '(Invalid Unicode 0xXXXXXXXX)'. If the std::string is in UTF16 encoding
...
// C std::string is considered different to any non-nullptr C std::string,
// including the empty std::string.
...
// Converts a wide C std::string to an std::string using the UTF-8 encoding.
// nullptr will be converted to "(null)".
std::string String::ShowWideCString(const wchar_t * wide_c_str)
{
    if (wide_c_str == nullptr)  return "(null)";

    return ToString(internal::WideStringToUtf8(wide_c_str, -1));
}
...
// C std::string is considered different to any non-nullptr C std::string,
// including the empty std::string.
bool String::CStringEquals(const wchar_t * lhs, const wchar_t * rhs)
{
    if (lhs == nullptr)
        return rhs == nullptr;
...
// On MacOS X, it uses towlower, which also uses LC_CTYPE category of the
// current locale.
bool String::CaseInsensitiveCStringEquals(const wchar_t* lhs, const wchar_t* rhs)
{
    if (lhs == nullptr)
        return rhs == nullptr;
...
// Any std::string is considered to end with an empty suffix.
...
static CharFormat PrintAsCharLiteralTo(Char c, std::ostream & os)
{
    ...
    case L'\v':
        ...
        break;
    default:
        if (IsPrintableAscii(static_cast<wchar_t>(c)))
        {
...
static void PrintCharsAsStringTo(const CharType* begin, size_t len, std::ostream & os)
{
    ...
    for (size_t index = 0; index < len; ++index)
    {
        ...
        if (is_previous_hex && IsXDigit(cur))
        {
            ...
            // interpreted as another hexadecimal digit in its number. Break std::string to
            // disambiguate.
            os << "\" " << kQuoteBegin;
        }
