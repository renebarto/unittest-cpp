...
basic_ostream<char> & operator<<(basic_ostream<char> & os, EnumClassYesNoWithStreamOperator value)
{
    switch (value)
    {
...
basic_ostream<char> & operator<<(basic_ostream<char> & os, const MyClassWithStreamOperator & x)
{
    os << x.value;
    return os;
}
...
#if defined(WIN_MSVC)
const std::string expectedName = "class Suiteunit_test_cpp::SuitePrintValue::MyClass";
#elif defined(LINUX) || defined(DARWIN) || defined(WIN_MINGW)
const std::string expectedName = "Suiteunit_test_cpp::SuitePrintValue::MyClass";
#endif
